...
BOOST_AUTO_TEST_CASE(daic_run)
{
  ...
  BOOST_CHECK_CLOSE(out.lambda_c, 0.767749320733306, 0.00000000001);
  BOOST_CHECK_CLOSE(out.mu, 1.77166095878803e-09, 0.00000000001);
  BOOST_CHECK_CLOSE(out.k, 3.00003880280079, 0.00000000001);
  BOOST_CHECK_CLOSE(out.gamma, 0.000750314431394791, 0.00000000001);
  BOOST_CHECK_CLOSE(out.lambda_a, 0.000101614571827502, 0.00000000001);
  BOOST_CHECK_CLOSE(out.loglik, 27.3882792747044, 0.00000000001);
  BOOST_CHECK_EQUAL(out.df, 5);
  BOOST_CHECK_EQUAL(out.conv, false);
}
