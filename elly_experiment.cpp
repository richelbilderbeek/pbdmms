...
void elly::experiment::run_daisie()
{
  ...
  const double init_lambda_c{p.get_clado_is().get()}; //Which lambda here, island or mainland?
  const double init_mu{p.get_ext_is().get()}; //Which rate here, island or mainland?
  const int init_k{p.get_carryingcap_is().get().get()}; //Which carrying capacity here, island or mainland?
