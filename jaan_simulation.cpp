...
void Simulation::run(Parameters& p,
    ...
    for (int g = 0; g < p.get_max_generations(); ++g) {
        ...
        std::vector<double> female_viab_dist(p.get_pop_size());
        crt_female_viability(p, population, female_viab_dist);
        std::uniform_real_distribution<> mother_distribution(0,
             female_viab_dist[(static_cast<int>(female_viab_dist.size()) - 1)]);
...
void Simulation::crt_female_viability(Parameters& p,
    ...
    female_viab_dist[0] = exp(-0.5 * temp * temp);
    assert(female_viab_dist[0] >= 0);
    for (int i = 1; i < pop_size; ++i) {
        temp = (population[i].get_preference() - p.get_optimal_preference()) /
                p.get_value_of_preference();
        female_viab_dist[i] = female_viab_dist[i-1] + exp(-0.5 * temp * temp);
        assert(female_viab_dist[i] >= female_viab_dist[i-1]);
    }
}
...
int Simulation::mother_choosing(Parameters& p,
                                ...
                                const double chosen) {
    const int pop_size{static_cast<int>(p.get_pop_size())};
    if (chosen > female_viab_dist[(static_cast<int>(female_viab_dist.size()) - 1)])
...
std::vector<Individual> Simulation::create_next_gen(Parameters& p,
    ...
    for (int i = 0; i < pop_size; ++i) {
        ...
        const int mother = mother_choosing(p, female_viab_dist, chosen);
