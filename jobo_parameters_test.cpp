...
int jobo::parameters_test() noexcept
{
  ...
  const int seed{38};
  parameters p(n_loci, population_size,seed);
  ...
  // test of parameters copy and equality
    const parameters a(42,1,38);
    const parameters b(a); //Copy
    const parameters c( 0,1,38);
    const parameters d(42,0,38);
    if (a != a) ++n_fails;
    if (a != b) ++n_fails;
    if (a == c) ++n_fails;
    if (a == d) ++n_fails;
