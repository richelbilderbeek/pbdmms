...
{
  if (g == gausser_implementation::lut) return "lut";
  if (g == gausser_implementation::raw) return "raw";
  throw std::invalid_argument("cannot convert gausser_implementation to string");
}

std::ostream& sado::operator<<(std::ostream& os, const gausser_implementation g) noexcept
{
  os << to_str(g);
  return os;
}
