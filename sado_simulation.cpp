...
sado::population sado::create_next_generation(const population &pop,
                                              const parameters &p) {
...
sado::population sado::create_next_generation_overlapping(population pop,
                                                          const parameters &p) {
...
sado::population sado::create_next_generation_seperate(const population &pop,
                                                       const parameters &p) {
  if (pop.empty()) {
    return pop;
  }
...
double sado::calc_comp(const population &pop, const double xi,
                       const parameters &p) noexcept {
  return std::accumulate(std::begin(pop), std::end(pop), -1.0,
                         [p, xi](double init, const indiv &i) {
                           return init + p.get_gausser_sc()(xi - i.get_x());
                         });
}

sado::offspring sado::create_kids(const population &pop, const indiv &mother,
...
sado::offspring sado::try_to_create_kids(const population &pop, const int index,
                                         const parameters &p) {
  assert(index < static_cast<int>(pop.size()));
...
std::vector<double> sado::get_attractivenesses(const population &pop,
                                               const double pi, const double xi,
                                               const parameters &p) {
